...
soi_lc <- soi_areas %>%
  ...
  # add EU27 total
  bind_rows(soi_eu %>% dplyr::select(-cum_area) %>% mutate(CNTR_CODE = "EU27") %>% pivot_wider(names_from = soi, values_from = area, values_fill = 0))
knitr::kable(soi_cntr, caption = "Areas of each braod land cover class affected by a given number of coinciding issues.")
...
soi_lc <- soi_areas %>%
  ...
  # add total by class
  rowwise() %>%
  mutate(total = sum(c_across(`0`:`11`))) %>%
  arrange(LC_name)
...
```{r soilcplot, fig.cap = "Frequency of number of coinciding issues in the EU27 by land cover class (excluding oversea territories)."}
soi_lc_pc <- soi_lc %>%
  mutate(across(`0`:`11`, ~round(.x/total*100))) %>%
  ungroup() %>%
...
```{r soilcmplot, fig.cap = "Distribution statistics on number of coinciding issues for land cover classes in the EU27. Small black dot is arithmetic mean; cross (x) is mode; red dot is median; vertical line ends are first and fourth quartiles."}
...
```{r issueslcplot, fig.cap="Percentage of area in EU27 subject to land change issues by land cover class.}
issues_lc_pc <- issues_lc %>%
  ...
  mutate(is_strat = if_else(issue %in% c("pop_issue", "rusle_issue", "wind_erosion_issue", "eutrofication_issue", "nitrogen", "acidification_issue"), true = "Stratified", false = "Not stratified"))
...
knitr::kable(soi_lpd %>% dplyr::select(-LPD), 
             caption = "Areas of each braod land cover class affected by a given number of coinciding issues.",
             col.names = c("Land Cover class", names(soi_lpd)[-1][-1]))

```
```{r soilpdplot, fig.cap = "Frequency of number of coinciding issues in the EU27 by land productivity dynamics class (excluding oversea territories)."}
soi_lpd_pc <- soi_lpd %>%
  mutate(across(`0`:`11`, ~round(.x/total*100))) %>%
  ungroup() %>%
...
```{r soilpdmplot, fig.cap = "Distribution statistics on number of coinciding issues for land productivuty dynamics classes in the EU27. Small black dot is arithmetic mean; cross (x) is mode; red dot is median; vertical line ends are first and fourth quartiles."}
...
soilpdmplot
```
Areas with both decreasing and increasing LPD are mostly (media, red dot) associated with three land change issues (need to subtract 1 from decreasing). Areas with stable LPD tend to have one issue less.
...
```
```{r issueslpdplot, fig.cap="Percentage of area in EU27 subject to land change issues by LPD class.}
